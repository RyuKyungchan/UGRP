...
def Data_Load_Plot(datapath):
    ...
    freqs, _, _, psd_Contaminated = FFT(Contaminated, fs=24414, single_sided=True)
...
def FFT(data, fs=24414, single_sided=True):
...
def Data_Load_Plot_previous(datapath):
    ...
    ### Frequency domain Plottig ###  
    freqs, _, _, Contaminated_psd = FFT(Contaminated, fs=24414, single_sided=True)
    _, _, _, Clean_psd = FFT(Clean, fs=24414, single_sided=True)
